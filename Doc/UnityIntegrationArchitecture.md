...
落地记录（VB-UX-16，2026-08-31）：宿主侧每会话一个 `GraphExecutionDebugController`，持有独立于 DAP 的 Runtime Bridge 连接（并行观察语义的落地）；Webview 工具栏「执行调试」入口 + 调试面板（实例列表/跟踪中/时间轴），`GraphDebugContext` 节点高亮、数据边最近值标签与经过边流光，回放游标支持事件级/帧级步进，epoch 重置经状态重建再水合；Webview 以 `graphDebugAck` 消息回执事件数/游标/当前节点/模式供测试断言（沿用 reveal ack 先例）。E2E：宿主测试（假 Runtime 实例 + 真实 Graph 编辑器）与 Runtime E2E 第 6 测（真实 Unity Play 驱动采集门面 → Encounter.vbflow 页面订阅/实时 ack/退订停采/切换）全过。