...
There is no standalone CLI. The current server does not implement Unity Catalog export, Unity import, Runtime compilation, Debug, Entity Documents, or Provider execution.
...
The fixed projects under `TestData/GraphSemanticProject` and `TestData/TableSemanticProject` are used by semantic and stdio integration tests:
