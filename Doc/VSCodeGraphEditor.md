...
- Render flow inputs and outputs immediately after the node type, before property editors and data ports. Property-bound data inputs stay beside their editor, while remaining data ports follow the property area.
...
- Add, edit, drag-reorder, and remove instance-level dynamic ports directly on a node. Each row uses a grip handle and compact delete action; dropping a row commits one reorder operation while preserving endpoint IDs. `Alt+↑/↓` on the grip provides keyboard reordering. Deleting a port removes its related edges in the same operation.
