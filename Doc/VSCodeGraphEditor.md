...
- Drag the left mouse button on empty canvas space to box-select every partially intersected node; drag the middle mouse button, or hold Space while left-dragging, to pan. Multi-selected nodes and edges are deleted as one Graph Operation batch. Final semantic validation prevents deleting required Graph Type nodes.
