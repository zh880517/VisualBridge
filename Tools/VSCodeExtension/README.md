...
The VisualBridge Activity Bar contains the `Documents` view. It groups the four editor categories by Project Document Type, searches titles/stable IDs/paths/diagnostics/references, creates documents, validates the workspace, publishes Problems, reveals outgoing references, lists incoming references, and expands physical CSV partition sources without replacing the native Explorer. Resolved `table.row` reference items can preview and atomically rename the target key plus every incoming Graph, Entity, Structured, or Table occurrence. See `Doc/DocumentBrowser.md` and `Doc/ProjectRefactoring.md`.
