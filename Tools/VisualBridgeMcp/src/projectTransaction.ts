...
export interface ProjectTransactionPrecondition {
  readonly path: string;
  readonly absolutePath: string;
...
async function commitWrites(
  ...
  if (writes.length === 0) return { sources: [] };
  const transactionId = randomUUID();
  const entries: JournalEntry[] = [];
  let journalWritten = false;
  let maintenance: ProjectTransactionCommitResult["maintenance"];
  try {
    if (new Set(writes.map((write) => write.absolutePath)).size !== writes.length) {
      throw new ProjectTransactionFailure(
        "transaction.duplicateTarget",
        "A project transaction cannot write the same physical source more than once.",
      );
    }
    ...
    for (const write of [...writes].sort((left, right) => left.path.localeCompare(right.path))) {
      ensureInside(projectRoot, write.absolutePath, write.path);
      ensureTargetMatchesLogicalPath(projectRoot, write.path, write.absolutePath);
      const current = await readFile(write.absolutePath);
      const beforeHash = hashBytes(write.before);
      if (hashBytes(current) !== beforeHash) {
        throw new ProjectTransactionConflict(
          "baseHashMismatch",
          `Source '${write.path}' changed before the transaction was staged.`,
          { path: write.path, expectedHash: beforeHash, actualHash: hashBytes(current) },
        );
      }
      const suffix = `.visualbridge-${transactionId}`;
      const temporaryPath = `${write.absolutePath}${suffix}.tmp`;
      const backupPath = `${write.absolutePath}${suffix}.rollback`;
      const targetStat = await stat(write.absolutePath);
      const temporaryHandle = await open(temporaryPath, "wx", targetStat.mode);
      try {
        await temporaryHandle.writeFile(write.after);
      ...
      entries.push({
        ...
        backupPath,
        beforeHash,
        afterHash: hashBytes(write.after),
      });
    }
    await writeJournal(projectRoot, { version: 1, transactionId, phase: "prepared", entries });
    journalWritten = true;
    await verifyPreconditions(preconditions);
    for (const entry of entries) {
      const currentHash = await readHash(entry.absolutePath);
    ...
    for (const entry of entries) {
      const persistedHash = await readHash(entry.absolutePath);
      if (persistedHash !== entry.afterHash) {
        throw new ProjectTransactionFailure(
          "transaction.verificationFailed",
          `Persisted source '${entry.path}' does not match the staged transaction.`,
          { path: entry.path, expectedHash: entry.afterHash, actualHash: persistedHash },
        );
      }
    }
    await writeJournal(projectRoot, { version: 1, transactionId, phase: "committed", entries });
    try {
...
async function verifyPreconditions(preconditions: readonly ProjectTransactionPrecondition[]): Promise<void> {
  for (const precondition of [...preconditions].sort((left, right) => left.path.localeCompare(right.path))) {
    const actualHash = await readHash(precondition.absolutePath);
    if (actualHash !== precondition.hash) {
      throw new ProjectTransactionConflict(
        "dependencyChanged",
        `Transaction dependency '${precondition.path}' changed.`,
        { path: precondition.path, expectedHash: precondition.hash, actualHash },
      );
    }
  }
}
  ...
  if (journal.phase === "committed") {
    ...
    for (const entry of journal.entries) {
      ...
      if (targetHash !== entry.afterHash) {
        failures.push(`${entry.path}: expected committed hash '${entry.afterHash}', found '${targetHash ?? "missing"}'.`);
      }
    }
    ...
    try {
      ...
      if (backupHash === undefined) {
        if (targetHash === undefined || targetHash === entry.afterHash) {
          throw new Error(`Missing rollback source; target hash is '${targetHash ?? "missing"}'.`);
        }
        // No replacement backup means this entry was never replaced. Preserve either the baseline
        // or an external writer's bytes and discard only this transaction's staged source.
      } else {
        ...
        if (await readHash(entry.absolutePath) !== entry.beforeHash) {
...
function parseJournal(bytes: Buffer, projectRoot: string): TransactionJournal {
  try {
    const value = JSON.parse(new TextDecoder("utf-8", { fatal: true }).decode(bytes)) as TransactionJournal;
    if (value.version !== 1 || typeof value.transactionId !== "string"
    ...
    for (const entry of value.entries) {
      if (typeof entry.path !== "string" || typeof entry.absolutePath !== "string"
        ...
        || !isHash(entry.beforeHash) || !isHash(entry.afterHash)) {
    ...
    if (new Set(value.entries.map((entry) => entry.absolutePath)).size !== value.entries.length) {
...
function ensureTargetMatchesLogicalPath(projectRoot: string, logicalPath: string, absolutePath: string): void {
  if (logicalPath.length === 0 || logicalPath.includes("\\") || logicalPath.startsWith("/")
